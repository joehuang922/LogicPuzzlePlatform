"""CLI entry point: python -m puzzle_parsers.sudoku <image> [-o output.json]"""
from __future__ import annotations

import argparse
import json
import sys
from pathlib import Path

from puzzle_parsers.recognition import (
    ClaudeOcrBackend,
    EasyOcrBackend,
    GeminiOcrBackend,
    OcrBackend,
)
from puzzle_parsers.sudoku.parser import SudokuParser


def _make_backend(name: str) -> OcrBackend:
    if name == "gemini":
        return GeminiOcrBackend()
    elif name == "claude":
        return ClaudeOcrBackend()
    elif name == "easyocr":
        return EasyOcrBackend()
    else:
        raise ValueError(f"Unknown OCR backend: {name!r}. Choose 'gemini', 'claude', or 'easyocr'.")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Parse a standard sudoku puzzle image into JSON"
    )
    parser.add_argument("image", help="Path to the puzzle image file")
    parser.add_argument(
        "-o",
        "--output",
        help="Output JSON file path (default: stdout)",
        default=None,
    )
    parser.add_argument(
        "--backend",
        choices=["gemini", "claude", "easyocr"],
        default="gemini",
        help="OCR backend to use (default: gemini)",
    )
    parser.add_argument(
        "--debug",
        help="Directory to save intermediate debug images",
        default=None,
    )
    args = parser.parse_args()

    image_path = Path(args.image)
    if not image_path.exists():
        print(f"Error: Image file not found: {image_path}", file=sys.stderr)
        sys.exit(1)

    backend = _make_backend(args.backend)
    sudoku_parser = SudokuParser(ocr_backend=backend)
    board = sudoku_parser.parse_file(image_path, debug_dir=args.debug)

    output_json = json.dumps(board.model_dump(), indent=4)

    if args.output:
        output_path = Path(args.output)
        output_path.write_text(output_json + "\n")
        print(f"Output written to: {output_path}", file=sys.stderr)
    else:
        print(output_json)


if __name__ == "__main__":
    main()
