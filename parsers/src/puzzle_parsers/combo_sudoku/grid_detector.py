...
def _filter_to_n_lines(peaks: list[int], target: int, size: int) -> list[int]:
    ...
    best_group = _find_best_grid_group(peaks, target)
    if best_group is not None:
        return best_group

    # Fallback: take the target peaks closest to expected positions
...
def extract_cells_from_geometry(
    ...
    """Extract cell images from a per-subboard geometry (warped to a single board)."""
