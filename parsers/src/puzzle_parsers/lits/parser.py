...
from puzzle_parsers.lits.grid_detector import (
    LitsGeometry,
    classify_borders,
    detect_lits_grid,
)
from puzzle_parsers.lits.models import LitsBoard, LitsGrids
from puzzle_parsers.models import PuzzleData


class LitsParser(PuzzleParser):
    puzzle_type = "lits"

    def __init__(self, **kwargs) -> None:
        # LITS carries no numbers or symbols, so no OCR/recognizer is needed.
    ...
    def _parse_image(
        ...
        geom = detect_lits_grid(img_array, debug_dir=debug_dir)
        ...
        return LitsBoard(grids=LitsGrids(h=h_borders, v=v_borders))

    def validate(self, data: PuzzleData) -> bool:
        if data.puzzle_type != self.puzzle_type:
            return False
        try:
            board = LitsBoard(**data.grid)
            h = board.grids.h
            v = board.grids.v
            rows = len(h) + 1
    ...
    def to_json(self, board: LitsBoard, output_path: str | Path) -> None:
