"""Akari (Light Up) grid detection — thin orchestrator over shared grid_utils.

Akari boards are a plain rectangular grid of cells with solid grid lines and no
regions. We reuse ``detect_grid_lines`` (solid-line detection); there are no
thick internal borders to classify.
"""
...
def detect_akari_grid(
    image: NDArray, debug_dir: str | None = None
) -> AkariGeometry:
    """Detect the Akari grid: find border, warp, locate solid grid lines."""
