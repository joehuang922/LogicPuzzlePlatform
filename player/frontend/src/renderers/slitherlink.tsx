...
export const slitherlinkRenderer: PuzzleRenderer = {
  puzzleType: 5,

  render(puzzle: PuzzleDefinition, state: PuzzleState, onValuesChange?: (values: Record<string, number>) => void, onComplete?: () => void) {
    const canonRepr = (typeof puzzle.canonRepr === "string" ? JSON.parse(puzzle.canonRepr) : puzzle.canonRepr) as SlitherlinkCanon;
    const savedAnswer = state.playerGrid as unknown as SlitherlinkAnswer | undefined;
