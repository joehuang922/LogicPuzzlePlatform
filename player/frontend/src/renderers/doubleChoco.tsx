...
function extractUserValues(savedAnswer: DoubleChocoAnswer | undefined): Record<string, number> {
  if (!savedAnswer?.grids) return {};
  const values: Record<string, number> = {};
  const { h, v } = savedAnswer.grids;
...
export const doubleChocoRenderer: PuzzleRenderer = {
  puzzleType: 4,

  render(puzzle: PuzzleDefinition, state: PuzzleState, onValuesChange?: (values: Record<string, number>) => void, onComplete?: () => void) {
    const canonRepr = (typeof puzzle.canonRepr === "string" ? JSON.parse(puzzle.canonRepr) : puzzle.canonRepr) as DoubleChocoCanon;
    const savedAnswer = state.playerGrid as unknown as DoubleChocoAnswer | undefined;
