...
function parseCanon(json: string): LitsCanon | null {
  try {
    ...
    if (
      ...
      return parsed as LitsCanon;
    }
  } catch {}
  return null;
}

function createEmptyBoard(rows: number, cols: number): LitsCanon {
  const h: number[][] = Array.from({ length: rows - 1 }, () => Array(cols).fill(0));
  const v: number[][] = Array.from({ length: rows }, () => Array(cols - 1).fill(0));
  return { grids: { h, v } };
}

function dimsOf(canon: LitsCanon): { rows: number; cols: number } {
  const rows = canon.grids.h.length + 1;
  const cols = canon.grids.v[0].length + 1;
  return { rows, cols };
}

export default function LitsEditor({ initialJson, onComplete, onCancel }: LitsEditorProps) {
  ...
  const updateJson = useCallback((newCanon: LitsCanon) => {
    ...
    return (
      <div style={{ padding: "1rem", border: "2px solid #c33", borderRadius: 8, background: "#fff8f8" }}>
        <p style={{ color: "#c33", margin: "0 0 1rem" }}>
          Invalid LITS JSON. Fix the textarea or create a new board.
        </p>
      ...
      <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
        <div style={{ flexShrink: 0 }}>
          ...
          <div style={{ marginTop: "0.5rem", fontSize: "0.75rem", color: "#666" }}>
            <strong>Regions</strong> are the areas enclosed by thick borders. No cell symbols in LITS.
          </div>
        </div>
