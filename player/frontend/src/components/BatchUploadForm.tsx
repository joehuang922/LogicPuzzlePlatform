...
function computeDimensions(typeId: number, canon: Record<string, unknown>): { w?: number; h?: number } {
  ...
  if ((typeId === 3 || typeId === 4 || typeId === 5 || typeId === 7 || typeId === 8 || typeId === 9 || typeId === 10 || typeId === 11 || typeId === 12 || typeId === 13 || typeId === 14 || typeId === 16 || typeId === 17 || typeId === 18 || typeId === 22 || typeId === 24) && canon.cells) {
...
function BatchItemRow({
  ...
  const typeName = puzzleTypes.find((pt) => pt.id === puzzleType)?.name;
  const hasEditor = ["nurimaze", "sudoku", "combo-sudoku", "double-choco", "slitherlink", "nonogram", "masyu", "pencils", "nuritwin", "slalom", "shakashaka", "kakuro", "yajilin", "fillomino", "lits", "choco-banana", "number-link", "akari", "hell-golf", "tentaishow", "heyawake", "shikaku", "norinori", "nurikabe"].includes(typeName || "");
