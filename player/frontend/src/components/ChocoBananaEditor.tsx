...
interface ChocoBananaEditorProps {
  initialJson: string;
...
function parseCanon(json: string): ChocoBananaCanon | null {
  try {
    const parsed = JSON.parse(json);
    if (parsed.cells && Array.isArray(parsed.cells) && parsed.cells.length > 0) {
      return parsed as ChocoBananaCanon;
    }
  } catch {}
  return null;
}

function createEmptyBoard(rows: number, cols: number): ChocoBananaCanon {
  const cells: number[][] = Array.from({ length: rows }, () => Array(cols).fill(0));
  return { cells };
}

export default function ChocoBananaEditor({ initialJson, onComplete, onCancel }: ChocoBananaEditorProps) {
  ...
  const updateJson = useCallback((newCanon: ChocoBananaCanon) => {
    ...
    return (
      <div style={{ padding: "1rem", border: "2px solid #c33", borderRadius: 8, background: "#fff8f8" }}>
        <p style={{ color: "#c33", margin: "0 0 1rem" }}>
          Invalid choco-banana JSON. Fix the textarea or create a new board.
        </p>
      ...
      <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
        <div style={{ flexShrink: 0 }}>
          ...
          <div style={{ marginTop: "0.5rem", fontSize: "0.75rem", color: "#666" }}>
            <strong>Values:</strong> 0 = empty (no clue), positive integer = group-size clue
          </div>
        </div>
