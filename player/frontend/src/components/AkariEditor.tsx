...
function parseCanon(json: string): AkariCanon | null {
  try {
    const parsed = JSON.parse(json);
    if (parsed.cells && Array.isArray(parsed.cells)) {
      return parsed as AkariCanon;
    }
  } catch {}
  return null;
}

function makeEmptyCanon(rows: number, cols: number): AkariCanon {
  return {
    cells: Array.from({ length: rows }, () => Array(cols).fill(-1)),
  };
}

export default function AkariEditor({
  initialJson,
  onChange,
}: AkariEditorProps) {
  ...
  const updateJson = useCallback((newCanon: AkariCanon) => {
    ...
    return (
      ...
      >
        <p style={{ color: "#c33", margin: "0 0 1rem" }}>
          Invalid akari JSON. Fix the textarea below or create a new grid.
        </p>
